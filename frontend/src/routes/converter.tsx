...
function FieldLabel({ htmlFor, children }: { htmlFor: string, children: React.ReactNode }) {
    return <label htmlFor={htmlFor} className="text-[9px] font-semibold uppercase tracking-[0.12em] text-muted/70 leading-none">
        {children}
    </label>
}
...
export default function Converter() {
    ...
    const renderOption = (step: Step, option: any) => {
        ...
        switch(option.type) {
            case 'number':
                return <div key={option.key} className="flex flex-col gap-1.5">
    ...
    return <div className="min-h-[calc(100vh-3rem)] flex">
        ...
        >
            ...
            <div className={`flex-1 max-h-[calc(100vh-9rem)] overflow-y-auto border rounded-xl p-4 transition-colors ${draggingOver ? 'border-primary bg-primary/5' : 'border-border'}`}>
                {steps.length === 0
                        ...
                        return <div key={step.id} className="flex flex-col">
                            ...
                            <div
                                ...
                                onDrop={e => {
                                    ...
                                    if(stepIndex !== '') { return moveStep(Number(stepIndex), index) }
                                    const toolId = e.dataTransfer.getData('tool-id')
        ...
        <aside className="flex flex-col gap-2 w-[20vw] bg-bg-light border-l border-border p-6">
            <div className="flex items-center justify-between">
                ...
                {input && <button onClick={() => setInput('')} title="Clear input" className="text-muted hover:text-primary cursor-pointer transition">
            ...
            <div className="flex items-center justify-between mt-4">
                ...
                {!output.isError && output.value && <button onClick={handleCopyOutput} title="Copy output" className="text-muted hover:text-primary cursor-pointer transition">
